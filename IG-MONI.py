...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["🔍 Verification", "🚫 Ban Monitor", "♻️ Unban Monitor"]:
        ...
        if not is_prem:
            ...
            sessions = ig_pool.get_available_sessions_for_user(user_id, True)
            if not sessions:
                await update.message.reply_text(
                    "❌ No admin sessions available. Please contact the administrator."
                )
                return
    ...
    if action in ["verification", "ban", "unban"]:
        ...
        try:
            if not is_prem:
                ...
                client = await ig_pool.get_client(selected_session, context, None)
            ...
            if not client:
                if not is_prem:
                    ...
                    await update.message.reply_text(
                        "❌ Cannot access Instagram API with admin session. Admin notified."
                    )
                    await context.bot.send_message(
                        ADMIN_USER_ID,
                        f"⚠️ Admin session @{selected_session} failed. Please update."
                    )
                del user_states[user_id]
                return
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("confirm_"):
        ...
        is_prem = is_premium(user_id)
        if is_prem:
            assigned_worker = get_least_loaded_worker_for_user(user_id, True)
        else:
            assigned_worker = get_least_loaded_user_session_for_user(user_id)
        
        if not assigned_worker:
            if is_prem:
                await query.edit_message_text("❌ No admin session available. Please contact admin.")
            else:
...
async def monitor_loop(user_id: int, monitor_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if is_premium(user_id):
                client = await ig_pool.get_client(assigned_worker, context, None)
            else:
            ...
            if not client:
                ...
                if not is_premium(user_id):
                    ...
                    await context.bot.send_message(
                        ADMIN_USER_ID,
                        f"⚠️ Admin session @{assigned_worker} has expired. Please update it."
                    )
                db.cursor.execute("UPDATE monitors SET status='stopped' WHERE id=?", (monitor_id,))
                db.conn.commit()
                break
